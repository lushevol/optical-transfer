...
Build a practical offline, one-way optical data transfer tool that sends a single directory from machine A to machine B through:
...
- A sender CLI that packages data and launches a local web player in Chromium for fullscreen QR playback
- A receiver CLI that ingests one or more video files and reconstructs the directory offline

The system should be treated as a reliable transfer protocol over an unreliable optical channel. Video is not trusted as a reliable container.

## Confirmed Decisions

### Operating Environment

- Sender device: MacBook screen, high refresh supported
- Recording device: phone capable of 4K at 60 Hz
- Receiver device: macOS
...
- Sender is CLI-based
- Receiver is CLI-based
- Sender playback uses a local webpage in Chrome/Chromium
- Receiver works in offline batch mode from recorded video files
- Receiver may accept multiple videos and merge chunks by `session_id`
...
- Receiver should perform practical preprocessing, not only naive frame extraction
...
- The header must be sufficient for the receiver to identify session membership and start decryption setup
...
- Receiver may merge data from multiple input videos for the same session
...
- Better fit for an unreliable optical channel
...
The receiver should accept one or more input videos and merge recoverable data by `session_id`.
...
- The receiver has all required logical chunks for the object
...
The receiver must emit a detailed session report.
...
- `Chrome/Chromium` for the sender playback surface
