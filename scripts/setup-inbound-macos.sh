...
VENV_DIR="${REPO_ROOT}/.venv-inbound"
PYTHON_BIN="${PYTHON_BIN:-python3}"
PIP_INDEX_URL="${PIP_INDEX_URL:-https://pypi.org/simple}"

echo "Initializing inbound environment in ${VENV_DIR}"
...
if ! "${PYTHON_BIN}" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 9) else 1)'; then
  ...
  echo '  PYTHON_BIN=python3.9 ./scripts/setup-inbound-macos.sh' >&2
  exit 1
fi
...
python -m pip install --index-url "${PIP_INDEX_URL}" --only-binary=opencv-python -e '.[inbound]'

if ! command -v ffmpeg >/dev/null 2>&1; then
  cat <<EOF
warning: ffmpeg was not found in PATH.

Install it before running inbound. For example on macOS with Homebrew:
  brew install ffmpeg
EOF
  exit 1
fi
...
Notes:
  - the inbound now depends on the packaged OpenCV QR decoder runtime
EOF
