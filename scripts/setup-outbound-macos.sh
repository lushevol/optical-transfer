...
VENV_DIR="${REPO_ROOT}/.venv-outbound"
PYTHON_BIN="${PYTHON_BIN:-python3}"

echo "Initializing outbound environment in ${VENV_DIR}"
...
if ! "${PYTHON_BIN}" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 9) else 1)'; then
  ...
  echo '  PYTHON_BIN=python3.9 ./scripts/setup-outbound-macos.sh' >&2
  exit 1
fi
...
Notes:
  ...
  - the outbound now renders real QR frames via the packaged qrcode dependency
EOF
